 ...
 */
void RadioRx( uint32_t timeout_N10ms );
...
@near uint8_t MaxPayloadLength = 0xFF;

@near uint32_t TxTimeout = 0;
@near uint32_t RxTimeout = 0;

@near bool RxContinuous = false;


@near PacketStatus_t RadioPktStatus;
@near uint8_t RadioRxPayload[255];
#else
 ...
 */
static RadioEvents_t* pRadioEvents;
...
void RadioInit( RadioEvents_t *events )
{
    ...
    SX126xSetRegulatorMode(USE_DCDC);/*1: DC_DC+LDO used for STBY_XOSC,FS, RX and TX modes  0x96*/
...
//     TimerInit( &RxTimeoutTimer, RadioOnRxTimeoutIrq );
    ...
    TmrCfgFnct(RX_TIMEOUT_TMRNUM, RadioOnRxTimeoutIrq);
    TmrCfgFnct(TX_TIMEOUT_TMRNUM, RadioOnTxTimeoutIrq);
...
//     DelayMs( 1 );
// 
//     carrierSenseTime = TimerGetCurrentTime( );/*返回的应该是ms单位的数值*/
// 
...
void RadioSend( uint8_t *buffer, uint8_t size )
{
    ...
    TmrSetTime(TX_TIMEOUT_TMRNUM, TxTimeout);
    TmrStart(TX_TIMEOUT_TMRNUM);
}
...
//         TimerStart( &RxTimeoutTimer );
		TmrSetTime(RX_TIMEOUT_TMRNUM, timeout_N10ms);
		TmrStart(RX_TIMEOUT_TMRNUM);
    }
...
//         TimerSetValue( &RxTimeoutTimer, timeout );
//         TimerStart( &RxTimeoutTimer );
		TmrSetTime(RX_TIMEOUT_TMRNUM, TxTimeout);
		TmrStart(RX_TIMEOUT_TMRNUM);
    }
...
//     TimerSetValue( &RxTimeoutTimer, (uint32_t)(time  * 1e3) );
//     TimerStart( &RxTimeoutTimer );

	TmrSetTime(RX_TIMEOUT_TMRNUM, (uint32_t)(time * 1e3));
	TmrStart(RX_TIMEOUT_TMRNUM);
}
...
void RadioOnTxTimeoutIrq(void)
{
    if( ( pRadioEvents != NULL ) && ( pRadioEvents->TxTimeout != NULL ) )
    {
        pRadioEvents->TxTimeout( );
    }
}

void RadioOnRxTimeoutIrq(void)
{
    if( ( pRadioEvents != NULL ) && ( pRadioEvents->RxTimeout != NULL ) )
    {
        pRadioEvents->RxTimeout( );
    }
}

// void RadioOnDioIrq( void )
// {
//     printf("RadioOnDioIrq_CallBack!\r\n");
//     IrqFired = true;
//     RadioIrqProcess();
// }
...
//         enableInterrupts( );     
        ...
        {
            printf("RadioIrqProcess:IRQ_TX_DONE\r\n");
//            TimerStop( &TxTimeoutTimer );
            TmrStop(TX_TIMEOUT_TMRNUM);
            //!< Update operating mode state to a value lower than \ref MODE_STDBY_XOSC
            SX126xSetOperatingMode( MODE_STDBY_RC );
            if( ( pRadioEvents != NULL ) && ( pRadioEvents->TxDone != NULL ) )
            {
                pRadioEvents->TxDone( );
            }
        }
        ...
        if( ( irqRegs & IRQ_RX_DONE ) == IRQ_RX_DONE )
        {
            ...
            printf("RadioIrqProcess:IRQ_RX_DONE\r\n");
//          TimerStop( &RxTimeoutTimer );
            TmrStop(RX_TIMEOUT_TMRNUM);
            ...
            if( ( pRadioEvents != NULL ) && ( pRadioEvents->RxDone != NULL ) )
            {
                pRadioEvents->RxDone( RadioRxPayload, size, RadioPktStatus.Params.LoRa.RssiPkt, RadioPktStatus.Params.LoRa.SnrPkt );
            }
        }

        if( ( irqRegs & IRQ_CRC_ERROR ) == IRQ_CRC_ERROR )
        {
            printf("RadioIrqProcess:IRQ_CRC_ERROR\r\n");
            if( RxContinuous == false )
            {
                //!< Update operating mode state to a value lower than \ref MODE_STDBY_XOSC
                SX126xSetOperatingMode( MODE_STDBY_RC );
            }
            if( ( pRadioEvents != NULL ) && ( pRadioEvents->RxError ) )
            {
                pRadioEvents->RxError( );
            }
        }
        ...
        if( ( irqRegs & IRQ_CAD_DONE ) == IRQ_CAD_DONE )
        {
            ...
            //!< Update operating mode state to a value lower than \ref MODE_STDBY_XOSC
            SX126xSetOperatingMode( MODE_STDBY_RC );
            if( ( pRadioEvents != NULL ) && ( pRadioEvents->CadDone != NULL ) )
            {
                if ((irqRegs & IRQ_CAD_ACTIVITY_DETECTED) == IRQ_CAD_ACTIVITY_DETECTED)
                {
                    printf("RadioIrqProcess:IRQ_CAD_DETECTE!\r\n");
                }
                else
                {
                    printf("RadioIrqProcess:IRQ_CAD_NODETECTE!\r\n");
                }
                pRadioEvents->CadDone( ( ( irqRegs & IRQ_CAD_ACTIVITY_DETECTED ) == IRQ_CAD_ACTIVITY_DETECTED ) );
            }
        }

        if( ( irqRegs & IRQ_RX_TX_TIMEOUT ) == IRQ_RX_TX_TIMEOUT )
        {
            printf("RadioIrqProcess:IRQ_RX_TX_TIMEOUT\r\n");
...
//                TimerStop( &RxTimeoutTimer );
        ...
        if( ( irqRegs & IRQ_PREAMBLE_DETECTED ) == IRQ_PREAMBLE_DETECTED )
        {
            SX126xWakeup();
            RadioStandby();
            printf("RadioIrqProcess:IRQ_PREAMBLE_DETECTED\r\n");
            RadioRx(1500);
            //__NOP( );
        }
        ...
        {
// 			SX126xSetDioIrqParams(IRQ_RX_DONE | IRQ_HEADER_ERROR , IRQ_RX_DONE | IRQ_HEADER_ERROR ,
// 				IRQ_RADIO_NONE, IRQ_RADIO_NONE);
            printf("RadioIrqProcess:IRQ_SYNCWORD_VALID\r\n");
            //__NOP( );
        }

        if( ( irqRegs & IRQ_HEADER_VALID ) == IRQ_HEADER_VALID )
        {
            printf("RadioIrqProcess:IRQ_HEADER_VALID\r\n");
            //__NOP( );
        }

        if( ( irqRegs & IRQ_HEADER_ERROR ) == IRQ_HEADER_ERROR )
        {
            printf("RadioIrqProcess:IRQ_HEADER_ERROR\r\n");
//            TimerStop( &RxTimeoutTimer );
            TmrStop(RX_TIMEOUT_TMRNUM);
            if( RxContinuous == false )
            {
