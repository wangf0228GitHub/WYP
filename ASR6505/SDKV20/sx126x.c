...
#define USE_TCXO 1
void SX126xInit(void)//( DioIrqHandler dioIrq )
{
    SX126xReset( );
...
#ifdef USE_TCXO
{
    ...
    SX126xSetDio3AsTcxoCtrl(TCXO_CTRL_2_2V, SX126xGetBoardTcxoWakeupTime( ) << 6 ); // convert from ms to SX126x time base
...
/*与上面的  SX126xSetRx 函数不同的仅仅是增大了RX的接收功率而已*/
...
/*测试用.lora模式会一直生成前导码向外发送*/
void SX126xSetTxInfinitePreamble( void )
{
    SX126xWriteCommand( RADIO_SET_TXCONTINUOUSPREAMBLE, 0, 0 );
}
/*0x00 Timer is stopped upon Sync Word or Header detection*/
/*0x01 Timer is stopped upon preamble detection*/
...
/*这个函数如果设置为0 表示不必接收到所有的设置好的前导码*/  
/*                  1 表示必须接收完毕所有的设置好的前导码*/  
